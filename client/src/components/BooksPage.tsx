...
import { BookResponse } from "../../../server/src/types";
  ...
  return (
    <div className="max-w-7xl mx-auto py-16 px-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
        {books?.map((book) => (
          <div key={book.id} className="relative rounded-3xl bg-white/80 dark:bg-gray-900/80 border shadow-xl p-6 hover:shadow-2xl">
            <div className="mb-4">
              <h3 className="text-2xl font-bold text-purple-800 dark:text-pink-300 mb-2">{book.title}</h3>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                ✍️ <span className="font-semibold text-indigo-600 dark:text-indigo-400">{book.authorName}</span> · {book.publishedYear}
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-300 mt-1">Genres: {book.genres.join(", ")}</p>
            </div>
